ALTER TABLE public.paper_trades
  ADD COLUMN IF NOT EXISTS strategy_epoch text NOT NULL DEFAULT 'v1';

CREATE INDEX IF NOT EXISTS paper_trades_epoch_idx
  ON public.paper_trades (user_id, strategy_epoch, status);

DROP FUNCTION IF EXISTS public.edge_report();

CREATE OR REPLACE FUNCTION public.edge_report(p_epoch text DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
...
GRANT EXECUTE ON FUNCTION public.edge_report(text) TO authenticated;